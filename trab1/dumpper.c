...
{
	int num =comando&127, aux = (comando>>12)&7, aux2 = (comando>>30)&1;
...
char *parametros(int comando, int *param, char *saida)
{
	...
	else if(param[1]==2)
	{
		...
		if(temAntes)
			join(saida, ", ",0);
		join(saida,toString(comando>>12,temp),0);
		temAntes=1;
...
void comandos(unsigned char *prog)
{
	...
	*/
	...
	int posStrtab = indx + 1;
	int posSymbols = lerLittNumEm(prog,posTabSec + (posSymtab*40)+16,4);
	int posText = lerLittNumEm(prog,posTabSec + (1*40)+16,4); //posicao de text (nao da secao)
	int virtualPosText = lerLittNumEm(prog, posTabSec + (1*40)+12,4);
	int posStr = lerLittNumEm(prog,posTabSec + (posStrtab*40)+16,4);
	//int posStrSec = lerLittNumEm(prog,posTabSec + (indx*40)+16,4);
	int tamSym = lerLittNumEm(prog,posTabSec+(40*posSymtab)+20,4)/16;
	int tamText = lerLittNumEm(prog,posTabSec + (1*40)+20,4);
	char saida[100];
...
{
	char assinatura[3];
	getString(prog,1,3,assinatura);
	mostrar("%s:\tfile format ",argv[2]);
	mostrar("%s",assinatura);
	mostrar("%s-",prog[4]==1?"32":"64");
